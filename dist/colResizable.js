...
(function ($) {
  ...
  var createGrips = function (t) {
    ...
    if (t.opt.postbackSafe.enabled && t.opt.postbackSafe.refreshStorage) {
      memento(t, null);
    }
  ...
  var memento = function (t, th) {
    ...
    if (th) {										//in deserialization mode (after a postback)
      ...
      if (t.opt.flush) {
        saveStatus['columns-width'] = "";
        return;
      } 	//if flush is activated, stored data is removed
      if (!saveStatus['columns-width']) {
        return;
      }
  ...
  var syncCols = function (t, i, isOver) {
    ...
    var w = c.w + inc;
    w = (w <= t.opt.minWidth) ? t.opt.minWidth : w;
    ...
    if (t.f) { //if fixed mode
      ...
      t.css('width', t.w + inc + PX);
      t.css('max-width', t.w + inc + PX);
    }
    if (isOver) {
      c.w = w;
      c2.w = t.f ? w2 : c2.w;
