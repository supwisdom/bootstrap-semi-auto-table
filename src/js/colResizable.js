...
(function($){
	...
	var init = function( tb, options){
		...
		t.p = t.opt.postbackSafe.enabled; 							//short-cut to detect postback safe
    t.key = t.opt.postbackSafe.key;
    ...
    if (!t.opt.postbackSafe.refreshStorage) {
		  if(t.p && S && S[t.key] && JSON.parse(S[t.key])['columns-width']) memento(t, th);		//if 'postbackSafe' is enabled and there is data for the current table, its coloumn layout is restored
    }
    th.each(function(i){						//iterate through the table column headers
      ...
      if (t.p && S && S[t.key] && JSON.parse(S[t.key])['hidden-columns']) {
        ...
        c.w = JSON.parse(S[t.key])['hidden-columns'][coulmn_index];
      }
    ...
    var saveStatus = S[t.key] ? JSON.parse(S[t.key]) : {};

		var w,m=0,i=0,aux =[],tw;
		if(th){										//in deserialization mode (after a postback)
			t.cg.removeAttr("width");
			if(t.opt.flush){ saveStatus['columns-width'] =""; return;} 	//if flush is activated, stored data is removed
      if (!saveStatus['columns-width']) {return;}
			w = saveStatus['columns-width'].split(";");					//column widths is obtained`
			tw = w[t.ln+1];
      ...
      saveStatus['columns-width'] ="";				//clean up previous data
			for(;i < t.c.length; i++){	//iterate through columns
				w = t.c[i].width();		//width is obtained
        saveStatus['columns-width'] += w+";";		//width is appended to the sessionStorage object using ID as key
				m+=w;					//carriage is updated to obtain the full size used by columns
			}
      saveStatus['columns-width']+=m;							//the last item of the serialized string is the table's active area (width),
												//to be able to obtain % width value of each columns while deserializing
			if(!t.f) saveStatus['columns-width'] += ";"+t.width(); 	//if not fixed, table width is stored
      S[t.key] = JSON.stringify(saveStatus);
		}
	};
	...
	* @param 的撒{jQuery ref} t - table object
